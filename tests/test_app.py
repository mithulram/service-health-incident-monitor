...
class ServiceMonitorTests(unittest.TestCase):
    ...
    def test_simulated_server_error_reduces_error_budget(self):
        client = TestClient(create_app(MonitorState(), demo_mode=True, database_url=TEST_DB))
    ...
    def test_simulation_is_disabled_without_demo_mode(self):
        client = TestClient(create_app(MonitorState(), demo_mode=False, database_url=TEST_DB))
        response = client.post("/api/v1/simulate/request", json={"status_code": 503})
        self.assertEqual(response.status_code, 403)

    def test_isolated_apps_do_not_share_simulation_state(self):
        client_a = TestClient(create_app(MonitorState(), demo_mode=True, database_url=TEST_DB))
        client_b = TestClient(create_app(MonitorState(), demo_mode=True, database_url=TEST_DB))
